...
static void play_current_song(void) {
    ...
    char filepath[256];
    snprintf(filepath, sizeof(filepath), "%s/%s", MOUNT_POINT, g_state.song_list.filenames[g_state.current_idx]);
...
void app_main(void) {
    ...
    xTaskCreate(io_buttons_task, "buttons_task", 2048, NULL, 5, &buttons_task_handle);
    ...
    while (1) {
        ...
        // Since mplayer currently stops on EOF but doesn't auto-update status to STOPPED in our structure,
        // we might want to add a check here or update mplayer to notify.
